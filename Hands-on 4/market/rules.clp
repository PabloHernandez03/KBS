...
;; (defrule print-all-products
;;    ?product <- (product (name ?name) (category ?category) (part-number ?part) (price ?price))
;;    =>
;;    (printout t "Product Name: " ?name ", Category: " ?category ", Part Number: " ?part ", Price: $" ?price crlf))


;; (defrule cust-all-prods
;;    (customer (customer-id ?id) (name ?cn))
;;    (order (order-number ?order) (customer-id ?id))
;;    (line-item (order-number ?order) (part-number ?part))
;;    (product (part-number ?part) (name ?pn))
;;    =>
;;    (printout t ?cn " bought " ?pn " in order number " ?order "." crlf))
